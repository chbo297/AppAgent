...
/// 对话流面板的透明根容器。
///
/// 根容器按 `OpenAPPChatPanelGeometry.dragHandleAreaHeight` 分成拖拽手柄区和内容区域。内容区域背景负责样式与阴影，viewport
...
final class OpenAPPChatPanelView: UIView {
    ...
    /// 内容区顶部导航栏，与拖拽手柄区相互独立。
    let navigationBar = OpenAPPChatPanelNavigationBar()

    /// 高度由 `OpenAPPChatPanelGeometry.dragHandleAreaHeight` 定义的透明拖拽手柄区。
    ...
    private let backgroundView = OpenAPPChatPanelBackgroundView()
    ...
    private var appliedLayout: OpenAPPChatPanelContentLayout?
    ...
    private func applyContentLayoutIfNeeded() {
        ...
        let minimumDisplayHeight = self.minimumDisplayHeight
            ?? min(
                bounds.height,
                OpenAPPChatPanelGeometry.dragHandleAreaHeight + OpenAPPInputBar.barHeight
            )
        ...
        UIView.performWithoutAnimation {
            ...
            // 手动调试跟手尺寸、位置时，优先查看 OpenAPPChatPanelContentLayout 产出的这两个 frame。
        ...
        // 调整跟手圆角时修改 layout 的两个 cornerRadius；调整具体路径形状时修改 OpenAPPChatPanelShapePath。
        let shapePath = OpenAPPChatPanelShapePath.make(
    ...
    private func setup() {
        ...
        grabberView.layer.cornerRadius = OpenAPPChatPanelContentLayout.grabberSize.height / 2
...
private final class OpenAPPChatPanelBackgroundView: UIView {
    ...
    func applyAppearance(for traitCollection: UITraitCollection) {
        CATransaction.begin()
        CATransaction.setDisableActions(true)
        fillLayer.fillColor = OpenAPPAppearance.inputBarBackground
            .resolvedColor(with: traitCollection)
            .cgColor
        layer.shadowColor = OpenAPPAppearance.inputBarShadow
            .resolvedColor(with: traitCollection)
            .cgColor
...
private enum OpenAPPChatPanelShapePath {
    static func make(
        in bounds: CGRect,
        ...
        let topRadius = OpenAPPGeometry.clamp(topCornerRadius, 0, maximumRadius)
        let bottomRadius = OpenAPPGeometry.clamp(bottomCornerRadius, 0, maximumRadius)
