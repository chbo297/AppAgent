...
struct OpenAPPChatPanelContentLayout: Equatable {
    /// A/B 过渡曲线系数；当前配置对应线性变化。
    static let compactTransitionEaseOutCoefficient: CGFloat = 1

    /// compact 状态与 inputBar 对齐所需的左右间距。
    static let compactHorizontalInset = OpenAPPInputBarFramePolicy.horizontalInset
    ...
    init(
        ...
        let normalizedMinimumHeight = OpenAPPGeometry.clamp(minimumDisplayHeight, 0, bounds.height)
        let transitionStart = OpenAPPGeometry.clamp(
        ...
        if transitionSpan > 0.001 {
            linearCompactProgress = OpenAPPGeometry.clamp(
        ...
        let compactFrame = CGRect(
            ...
            height: min(OpenAPPInputBar.barHeight, contentAreaFrame.height)
        )
        ...
        let expandedCornerRadius = OpenAPPChatPanelGeometry.topCornerRadius
        let compactCornerRadius = OpenAPPInputBar.expandedCornerRadius
        topCornerRadius = min(
            maximumRadius,
            Self.interpolate(
